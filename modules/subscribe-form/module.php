<?php
namespace Nova_Elements\Modules\SubscribeForm;
...
class Module extends Module_Base {
	...
	public function init_ajax_handler(){
        ...
        $this->sys_messages = apply_filters( 'nova_elements/popups_sys_messages', array(
            'invalid_mail'                => esc_html__( 'Please, provide valid mail', 'nova-elements' ),
            'mailchimp'                   => esc_html__( 'Please, set up MailChimp API key and List ID', 'nova-elements' ),
            'internal'                    => esc_html__( 'Internal error. Please, try again later', 'nova-elements' ),
            'server_error'                => esc_html__( 'Server error. Please, try again later', 'nova-elements' ),
            'subscribe_success'           => esc_html__( 'Success', 'nova-elements' ),
        ) );

        add_action( 'wp_ajax_nova_subscribe_form_ajax', [ $this, 'subscribe_form_ajax' ] );
        add_action( 'wp_ajax_nopriv_nova_subscribe_form_ajax', [ $this, 'subscribe_form_ajax' ] );
    ...
    public function enqueue_frontend_localize_script(){
        wp_localize_script(
            'nova-elements',
            'nova_subscribe_form_ajax',
            array(
                'action' => 'nova_subscribe_form_ajax',
                'nonce' => wp_create_nonce('nova_subscribe_form_ajax'),
                'type' => 'POST',
                'data_type' => 'json',
                'is_public' => 'true',
    ...
    public function subscribe_form_ajax() {
        ...
        if ( ! wp_verify_nonce( isset($_POST['nonce']) ? $_POST['nonce'] : false, 'nova_subscribe_form_ajax' ) ) {
            $response = array(
        ...
        $api_key = get_option('nova_elements_mailchimp_api_key', '');
        $list_id = get_option('nova_elements_mailchimp_list_id', '');
        $double_opt_in = filter_var( get_option('nova_elements_mailchimp_double_opt_in', ''), FILTER_VALIDATE_BOOLEAN );
