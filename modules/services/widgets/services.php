<?php
namespace Nova_Elements\Modules\Services\Widgets;
 ...
 * Services Widget
 */
class Services extends Nova_Widget {
    ...
    protected function get_widget_title() {
        return esc_html__( 'Services', 'nova-elements' );
    }

    public function get_icon() {
        return 'novaelements-icon-26';
    }

    protected function _register_controls() {
        $css_scheme = apply_filters(
            'nova-elements/services/css-scheme',
            array(
                'instance'         => '.nova-services',
                'instance_inner'   => '.nova-services__inner',
                'header'           => '.nova-services__header',
                'cover'            => '.nova-services__cover',
                'figure'           => '.nova-services__figure',
                'content'          => '.nova-services__content',
                'icon'             => '.nova-services__icon',
                'title'            => '.nova-services__title',
                'title_icon'       => '.nova-services__title-icon',
                'title_text'       => '.nova-services__title-text',
                'desc'             => '.nova-services__desc',
                'button'           => '.nova-services__button',
                'button_icon'      => '.nova-services__button-icon',
            )
        );
        ...
        $this->add_control(
            ...
            array(
                ...
                'default' => esc_html__( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.', 'nova-elements' ),
                'dynamic' => array( 'active' => true ),
            )
        );
        ...
        $this->add_control(
            'header_position',
            array(
                'label'   => esc_html__( 'Header Position', 'nova-elements' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'top',
                'options' => array(
        ...
        $this->add_control(
            'icon_cover_location',
            array(
                'label'        => esc_html__( 'Display in header', 'nova-elements' ),
        ...
        $this->add_control(
            'use_title_icon',
            array(
                'label'        => esc_html__( 'Use title icon?', 'nova-elements' ),
                'type'         => Controls_Manager::SWITCHER,
                'label_on'     => esc_html__( 'Yes', 'nova-elements' ),
                'label_off'    => esc_html__( 'No', 'nova-elements' ),
                'return_value' => 'yes',
        ...
        $this->add_control(
            'title_icon',
            array(
                'label'       => esc_html__( 'Title Icon', 'nova-elements' ),
                'type'        => Controls_Manager::ICON,
                'label_block' => true,
                'file'        => '',
                'default'     => '',
        ...
        $this->add_responsive_control(
            'title_icon_margin',
            array(
                'label'      => __( 'Icon Margin', 'nova-elements' ),
                'type'       => Controls_Manager::DIMENSIONS,
                'size_units' => array( 'px', '%' ),
                'selectors'  => array(
        ...
        $this->add_control(
            'button_fullwidth',
            array(
                'label'        => esc_html__( 'Enable Button Fullwidth', 'nova-elements' ),
                'type'         => Controls_Manager::SWITCHER,
                'label_on'     => esc_html__( 'Yes', 'nova-elements' ),
                'label_off'    => esc_html__( 'No', 'nova-elements' ),
                'return_value' => 'yes',
    ...
    public function __generate_icon( $cover_location = false ) {
        ...
        if ( empty( $icon ) ) {
            ...
            $format = apply_filters( 'nova-elements/services/icon-format', '<div class="nova-services__icon"><div class="inner"><i><img src="%s"/ alt=""></i></div></div>' );
            if ( empty( $image['id'] ) ) {
            ...
            $format = apply_filters( 'nova-elements/services/icon-format', '<div class="nova-services__icon"><div class="inner"><i class="%s"></i></div></div>' );
        }
    ...
    public function __generate_title( $cover_location = false ) {
        ...
        if ( ! empty( $title_icon ) ) {
            $icon_html = sprintf( '<span class="nova-services__title-icon"><i class="%s"></i></span>', $title_icon );
        }

        if ( ! empty( $title ) ) {

            $title_html = sprintf( '<span class="nova-services__title-text">%s</span>', $title );
        }

        $title_tag = $this->get_settings_for_display( 'services_title_size' );

        $format = apply_filters( 'nova-elements/services/name-format', '<%3$s class="nova-services__title">%1$s%2$s</%3$s>' );
    ...
    public function __generate_description( $cover_location = false ) {
        ...
        $format = apply_filters( 'nova-elements/services/description-format', '<p class="nova-services__desc">%s</p>' );
    ...
    public function __generate_action_button( $cover_location = false ) {
        ...
        if ( filter_var( $use_icon, FILTER_VALIDATE_BOOLEAN ) ) {
            $icon_html = sprintf( '<i class="nova-services__button-icon %s"></i>', $button_icon );
        }

        $this->add_render_attribute( 'url', 'class', array(
            'elementor-button',
            'elementor-size-md',
            'nova-services__button',
            'nova-services__button--icon-' . $icon_position,
        ) );
        ...
        $format = apply_filters( 'nova-elements/services/action-button-format', '<a %1$s><span class="nova-services__button-text">%2$s</span>%3$s</a>' );
