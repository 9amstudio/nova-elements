<?php
namespace Nova_Elements\Modules\AdvancedMap\Widgets;
 ...
 * Advanced_Map Widget
 */
class Advanced_Map extends Nova_Widget {
    ...
    protected function get_widget_title() {
        return esc_html__( 'Advanced Map', 'nova-elements' );
    }
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section(
            ...
            array(
                'label' => esc_html__( 'Map Settings', 'nova-elements' ),
            )
        );

        $key = apply_filters('nova-elements/advanced-map/api', '');
        ...
        $default_address = esc_html__( 'PO Box 16122 Collins Street West Victoria 8007 Australia', 'nova-elements' );

        $this->add_control(
            'map_center',
            array(
                'label'       => esc_html__( 'Map Center', 'nova-elements' ),
        ...
        $this->add_control(
            'zoom',
            array(
                'label'      => esc_html__( 'Initial Zoom', 'nova-elements' ),
        ...
        $this->add_control(
            'scrollwheel',
            array(
                'label'   => esc_html__( 'Scrollwheel Zoom', 'nova-elements' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'false',
                'options' => array(
                    'true'  => esc_html__( 'Enabled', 'nova-elements' ),
                    'false' => esc_html__( 'Disabled', 'nova-elements' ),
                ),
            )
        );
        ...
        $this->add_control(
            'zoom_controls',
            array(
                'label'   => esc_html__( 'Zoom Controls', 'nova-elements' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'true',
                'options' => array(
                    'true'  => esc_html__( 'Show', 'nova-elements' ),
                    'false' => esc_html__( 'Hide', 'nova-elements' ),
                ),
            )
        );
        ...
        $this->add_control(
            'fullscreen_control',
            array(
                'label'   => esc_html__( 'Fullscreen Control', 'nova-elements' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'true',
                'options' => array(
                    'true'  => esc_html__( 'Show', 'nova-elements' ),
        ...
        $this->add_control(
            'street_view',
            array(
                'label'   => esc_html__( 'Street View Controls', 'nova-elements' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'true',
                'options' => array(
                    'true'  => esc_html__( 'Show', 'nova-elements' ),
        ...
        $this->add_control(
            'map_type',
            array(
                'label'   => esc_html__( 'Map Type Controls (Map/Satellite)', 'nova-elements' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'true',
                'options' => array(
                    'true'  => esc_html__( 'Show', 'nova-elements' ),
        ...
        $this->add_control(
            'drggable',
            array(
                'label'   => esc_html__( 'Is Map Draggable?', 'nova-elements' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'true',
                'options' => array(
                    'true'  => esc_html__( 'Yes', 'nova-elements' ),
                    'false' => esc_html__( 'No', 'nova-elements' ),
                ),
            )
        );
        ...
        $this->start_controls_section(
            'section_map_style',
            array(
                'label' => esc_html__( 'Map Style', 'nova-elements' ),
            )
        );
        ...
        $this->add_control(
            'map_height',
            array(
                'label'       => esc_html__( 'Map Height', 'nova-elements' ),
                ...
                'selectors' => array(
                    '{{WRAPPER}} .nova-map' => 'height: {{VALUE}}px',
                ),
            )
        );
        ...
        $this->add_control(
            ...
            array(
                ...
                'label_block' => true,
                'description' => esc_html__( 'You can add own map styles within your theme. Add file with styles array in .json format into nova-elements/google-map-styles/ folder in your theme. File must be minified', 'nova-elements' )
            )
        );
        ...
        $this->start_controls_section(
            'section_map_pins',
            array(
                'label' => esc_html__( 'Pins', 'nova-elements' ),
            )
        );
        ...
        $repeater->add_control(
            'pin_address',
            array(
                'label'       => esc_html__( 'Pin Address', 'nova-elements' ),
                'type'        => Controls_Manager::TEXT,
                'default'     => $default_address,
                'label_block' => true,
                'dynamic'     => array( 'active' => true ),
        ...
        $repeater->add_control(
            'pin_desc',
            array(
                'label'   => esc_html__( 'Pin Description', 'nova-elements' ),
        ...
        $repeater->add_control(
            'pin_image',
            array(
                'label'   => esc_html__( 'Pin Icon', 'nova-elements' ),
        ...
        $repeater->add_control(
            'pin_state',
            array(
                'label'   => esc_html__( 'Initial State', 'nova-elements' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'visible',
                'options' => array(
                    'visible' => esc_html__( 'Vsible', 'nova-elements' ),
                    'hidden'  => esc_html__( 'Hidden', 'nova-elements' ),
                ),
            )
        );
    ...
    public function _get_available_map_styles() {
        ...
        $key           = md5( 'nova-elements-' . NOVA_ELEMENTS_VER );
        $plugin_styles = get_transient( $key );

        if ( ! $plugin_styles ) {

            $plugin_styles = $this->_get_map_styles_from_path(
                NOVA_ELEMENTS_PATH . 'assets/js/lib/google-maps/'
            );
        ...
        $parent_styles = $this->_get_map_styles_from_path(
            get_template_directory() . '/' . nova_elements_template_path() . 'google-map-styles/'
        );

        if ( get_stylesheet_directory() !== get_template_directory() ) {
            $child_styles = $this->_get_map_styles_from_path(
                get_stylesheet_directory() . '/' . nova_elements_template_path() . 'google-map-styles/'
            );
        } else {
            $child_styles = array();
        }
        ...
        return array_merge(
            array( 'default' => esc_html__( 'Default', 'nova-elements' ) ),
            $plugin_styles,
            $parent_styles,
            $child_styles
        );
    ...
    public function get_location_coord( $location ) {
        ...
        $api_key = apply_filters('nova-elements/advanced-map/api', '');

        // Do nothing if api key not provided
        if ( ! $api_key ) {
            $message = esc_html__( 'Please set Google maps API key before using this widget.', 'nova-elements' );
        ...
        if ( ! $coord ) {
            ...
            $message = esc_html__( 'Coordinates of this location not found', 'nova-elements' );
    ...
    protected function render() {
        ...
        $init = apply_filters( 'nova-elements/addons/advanced-map/data-args', array(
            'center'            => $coordinates,
        ...
        printf(
            '<div class="nova-map" %1$s %2$s></div>',
    ...
    public function get_map_message( $message ) {
        return sprintf( '<div class="nova-map-message"><div class="nova-map-message__dammy-map"></div><span class="nova-map-message__text">%s</span></div>', $message );
    }
}
