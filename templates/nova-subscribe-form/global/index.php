...
$this->add_render_attribute( 'main-container', 'class', array(
	'nova-subscribe-form',
	'nova-subscribe-form--' . $layout . '-layout',
) );

$this->add_render_attribute( 'main-container', 'data-settings', $this->generate_setting_json() );

$instance_data = apply_filters( 'nova-elements/subscribe-form/input-instance-data', array(), $this );
...
$this->add_render_attribute( 'form-input',
	array(
		'class'       => array(
			'nova-subscribe-form__input nova-subscribe-form__mail-field',
		),
		'type'               => 'email',
		'name'               => 'email',
...
if ( filter_var( $use_icon, FILTER_VALIDATE_BOOLEAN ) ) {
	$icon_html = sprintf( '<i class="nova-subscribe-form__submit-icon %s"></i>', $button_icon );
}

?>
<div <?php echo $this->get_render_attribute_string( 'main-container' ); ?>>
	<form method="POST" action="#" class="nova-subscribe-form__form">
		<div class="nova-subscribe-form__input-group">
			<div class="nova-subscribe-form__fields">
				<input <?php echo $this->get_render_attribute_string( 'form-input' ); ?>><?php
					$this->generate_additional_fields();
				?></div>
			<?php echo sprintf( '<a class="nova-subscribe-form__submit elementor-button elementor-size-md" href="#">%s<span class="nova-subscribe-form__submit-text">%s</span></a>', $icon_html, $submit_button_text ); ?>
		</div>
		<div class="nova-subscribe-form__message"><div class="nova-subscribe-form__message-inner"><span></span></div></div>
	</form>
</div>
