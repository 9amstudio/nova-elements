...
$this->add_render_attribute( 'url', 'class', array(
	'elementor-button',
	'elementor-size-md',
	'nova-animated-box__button',
	'nova-animated-box__button--back',
	'nova-animated-box__button--icon-' . $position,
) );
...
<a <?php echo $this->get_render_attribute_string( 'url' ); ?>><?php
	echo $this->__html( 'back_side_button_text', '<span class="nova-animated-box__button-text">%s</span>' );

	if ( filter_var( $use_icon, FILTER_VALIDATE_BOOLEAN ) ) {
		echo $this->__html( 'button_icon', '<i class="nova-animated-box__button-icon %s"></i>' );
	}
?></a>
