...
$columns640 = $settings['columns_width640'];
...
if ( 'masonry' == $layout || 'grid' == $layout ) {
	$this->add_render_attribute( 'main-container', 'class', array(
		...
		! empty( $columns640 ) ? 'layout-width640-column-' . $columns640 : '',
	) );
}
...
<div <?php echo $this->get_render_attribute_string( 'main-container' ); ?>><?php
    ...
    if( 'grid' == $layout || 'list' == $layout ) {
        ...
        if(!empty($slider_options)){
            echo sprintf('<div class="nova-carousel elementor-slick-slider" data-slider_options="%1$s" dir="%2$s">', $slider_options, is_rtl() ? 'rtl' : 'ltr');
        }
