...
if ( 'grid' === $settings['layout_type'] ) {
	$col_class = nova_elements_tools()->col_classes( array(
        ...
        'mobp'  => $this->__get_html( 'columns_width640' )
	) );
}
...
?>
<div class="nova-images-layout__item <?php echo $col_class ?>">
	<div class="nova-images-layout__inner">
		<a <?php echo $this->get_render_attribute_string( $link_instance ); ?>>
			<div class="nova-images-layout__image">
				<?php
					if ( 'justify' === $settings['layout_type'] ) {
						echo $this->__loop_image_item( 'item_image', '<img class="nova-images-layout__image-instance" src="%1$s" data-width="%2$s" data-height="%3$s" alt="">' );
					} else {
						echo $this->__loop_item( array( 'item_image', 'url' ), '<img class="nova-images-layout__image-instance" src="%s" alt="">' );
					}
				?>
			</div>
			<div class="nova-images-layout__content">
					<?php
						echo $this->__loop_item( array( 'item_icon' ), '<div class="nova-images-layout__icon"><div class="nova-images-layout-icon-inner"><i class="%s"></i></div></div>' );
					?>

					<?php
						$title_tag = $this->__get_html( 'title_html_tag', '%s' );

						echo $this->__loop_item( array( 'item_title' ), '<' . $title_tag . ' class="nova-images-layout__title">%s</' . $title_tag . '>' );
						echo $this->__loop_item( array( 'item_desc' ), '<div class="nova-images-layout__desc">%s</div>' );
					?>

			</div>
		</a>
	</div>
	<div class="nova-images-layout__image-loader"><span></span></div>
</div>
